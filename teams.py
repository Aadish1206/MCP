...
from mcpgateway.middleware.rbac import get_db  # or your local get_db if defined there
...
@require_permission("teams.read")
async def list_team_members(team_id: str, user=Depends(get_current_user_with_permissions), current_user: EmailUserResponse = Depends(get_current_user), db: Session = Depends(get_db)) -> List[TeamMemberResponse]:
    """List team members.
...
@require_permission("teams.manage_members")
async def invite_team_member(team_id: str, request: TeamInviteRequest, current_user: EmailUserResponse = Depends(get_current_user), user=Depends(get_current_user_with_permissions), db: Session = Depends(get_db)) -> TeamInvitationResponse:
...
async def list_team_invitations(
    team_id: str,
    current_user: EmailUserResponse = Depends(get_current_user),               # for owner check
    user=Depends(get_current_user_with_permissions),                           # for RBAC decorator
...
async def accept_team_invitation(
    ...
    try:
        ...
        return TeamMemberResponse(
            id=mm.id,
            team_id=mm.team_id,
            user_email=mm.user_email,
            role=mm.role,
            joined_at=mm.joined_at,
            invited_by=mm.invited_by,
...
@require_permission("teams.manage_members")
async def cancel_team_invitation(invitation_id: str, current_user: EmailUserResponse = Depends(get_current_user), user=Depends(get_current_user_with_permissions),db: Session = Depends(get_db)) -> SuccessResponse:
...
async def request_to_join_team(
    ...
    try:
        ...
        # Use the email from the user-context dict
        ...
        if team.visibility != "public":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Can only request to join public teams",
            )
        ...
        # Create join request
        join_req = await team_service.create_join_request(
            team_id=team_id,
